...
//                                int32_t* tp)
__global__ void Triangle_Match1(plane* plane_g, int32_t* disparity_grid, int32_t *grid_dims,\
                                uint8_t* I1_desc, uint8_t* I2_desc, int32_t* P, \
                                int32_t plane_radius, bool right_image, float* D,  \
                                int32_t* tp)
{
...
//        __shared__ plane        plane_g_shared[320];
        __shared__ int32_t      disparity_grid_g[65* (WIDTH/GRID_SIZE)];
//        __shared__ int32_t      tp_g[WIDTH * 2];

        int u = u_start*20, v = v_start;
//printf("%d\n", u_start);
//        for( ; u < u_start*20 + 20; u++ )
//        {
//            tp_g[u] = tp[v * WIDTH*2 + u];
//        }
//printf("\n");
//        u = u_start*10/GRID_SIZE;
        ...
        {
            disparity_grid_g[u + i] = disparity_grid[v*65* (WIDTH/GRID_SIZE)\
                                        + u + i];
        }


                u = u_start; v = v_start;
                for(int i = 0; i < 160*32; i += 32 )
                {
                    I1_desc_share[u + i] = I1_desc[v * 320*16 + u + i];
                    I2_desc_share[u + i] = I2_desc[v * 320*16 + u + i];
                }
...
//            u = u_start*10, v = v_start;
//        for(; u < u_start*10 + 10; u++)
//        {
//            id = tp[2 * u + v * 2 * WIDTH + 1];
////            id = tp_g[2 * u+1];

//            plane_a_g[u] = plane_g[id].t1a;
//            plane_b_g[u] = plane_g[id].t1b;
//            plane_c_g[u] = plane_g[id].t1c;
//            plane_d_g[u] = plane_g[id].t2a;
////            printf("%f ",plane_g[id].t1a);
//        }
//printf("\n");
...
plane_1 = (plane*)malloc(sizeof(plane) * TRI_SIZE1);
plane_2 = (plane*)malloc(sizeof(plane) * TRI_SIZE2);
for(int i = 0; i < TRI_SIZE1; i++)
{
    plane_1[i].t1a = tri_1[i].t1a;
    plane_1[i].t1b = tri_1[i].t1b;
    plane_1[i].t1c = tri_1[i].t1c;
    plane_1[i].t2a = tri_1[i].t2a;
}
for(int i = 0; i < TRI_SIZE2; i++)
{
    plane_2[i].t1a = tri_2[i].t2a;
    plane_2[i].t1b = tri_2[i].t2b;
    plane_2[i].t1c = tri_2[i].t2c;
    plane_2[i].t2a = tri_2[i].t1a;
}
cudaMalloc((void **)&plane_g1, sizeof(plane) * TRI_SIZE1);
cudaMalloc((void **)&plane_g2, sizeof(plane) * TRI_SIZE2);
cudaMemcpy(plane_g1, plane_1, sizeof(plane) * TRI_SIZE1, cudaMemcpyHostToDevice);
cudaMemcpy(plane_g2, plane_2, sizeof(plane) * TRI_SIZE2, cudaMemcpyHostToDevice);
...
err = cudaFuncSetCacheConfig(Triangle_Match1,cudaFuncCachePreferShared);
if(cudaSuccess != err)
{
...
dim3 threads(32, 1);
dim3 grid(iDivUp(width, (threads.x * 10)), iDivUp(height,threads.y));
...
//Triangle_Match1 << <grid, threads, 0, stream1>> > (tri_gpu_1, disparity_grid_gpu_1, \
//             grid_dims_gpu, I_desc1, I_desc2, P_gpu, plane_radius, 0, D1_gpu, \
//             tp1_gpu);

Triangle_Match1 << <grid, threads, 0>> > (plane_g1, disparity_grid_gpu_1, \
             grid_dims_gpu, I_desc1, I_desc2, P_gpu, plane_radius, 0, D1_gpu, \
             tp1_gpu);
...
//Triangle_Match1 << <grid, threads, 0, stream2>> > (tri_gpu_2, disparity_grid_gpu_2, \
//    grid_dims_gpu, I_desc2, I_desc1, P_gpu, plane_radius, 1, D2_gpu, \
//                      tp2_gpu);
//Triangle_Match1 << <grid, threads, 0>> > (plane_g2, disparity_grid_gpu_2, \
//    grid_dims_gpu, I_desc2, I_desc1, P_gpu, plane_radius, 1, D2_gpu, \
//                      tp2_gpu);
