...
//     __constant__ int32_t grid_dims_g[3] =  {*grid_dims, *(grid_dims+1),*(grid_dims+2)};
...
//     __syncthreads();
    ...
    if ( (x > 2) && (x < width - 2)  && (y < height) ) {
        if (tp[2 * x + y * 2 * width] != -1) {

            id = tp[2 * x + y * 2 * width + 1];
            ...
            findMatch(x, y, plane_a, plane_b, plane_c, disparity_grid, grid_dims,
                I1_desc, I2_desc, P, plane_radius, valid, right_image, D, width, height, subsampling, match_texture, grid_size);
