...
void clElas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims)
{
  ...
  t2=clock();
  cout<<"descriptor cost:"<<t2-t1<<endl;
#ifdef PROFILE
  ...
  cout << "support size:"<<p_support.size()<<endl;
  ...
  t2=clock();
  cout <<"computesuppportmatch"<< t2-t1 << "ms" <<endl;
#ifdef PROFILE
  ...
  t2=clock();
  cout <<"computediparityplanes"<< t2-t1 << "ms" <<endl;
#ifdef PROFILE
  timer.start("Grid");
#endif
  t1=clock();
  ...
  t2=clock();
  cout <<"creategrid"<< t2-t1 << "ms" <<endl;
#ifdef PROFILE
  timer.start("Matching");
#endif
  //深度计算
  t1=clock();
  // clcomputeDisparity(p_support,tri_1,disparity_grid_1,grid_dims,desc1.I_desc,desc2.I_desc,D1);     
  ...
  // clcomputeDisparityright(p_support,tri_2,disparity_grid_2,grid_dims,desc1.I_desc,desc2.I_desc,D2);
  t2=clock();
  cout <<"computedisparity"<< t2-t1 << "ms" <<endl;
...
//vector<clElas::support_pt> clElas::clcomputeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc)
//{
//	int errNum;
//}
void clElas::clcomputeDisparity(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
								uint8_t* I1_desc,uint8_t* I2_desc,float* D)
{
	...
	//准备数据
	int *support = new int32_t[p_support.size()*3];
	for(int i=0;i< p_support.size();i++)
	{
		support[ (i * 3) + 0] = p_support[i].u;
		support[ (i * 3) + 1] = p_support[i].v;
		support[ (i * 3) + 2] = p_support[i].d;
	}
	int *tri_0   = new int32_t[tri.size() * 3];
	float *tri_1 = new float[tri.size() * 3];
	float *tri_2 = new float[tri.size() * 3];
	for(int i = 0;i < tri.size() ;i++)
	{
		tri_0[ (i * 3) + 0] = tri[i].c1;
		tri_0[ (i * 3) + 1] = tri[i].c2;
		tri_0[ (i * 3) + 2] = tri[i].c3;
		tri_1[ (i * 3) + 0] = tri[i].t1a;
		tri_1[ (i * 3) + 1] = tri[i].t1b;
		tri_1[ (i * 3) + 2] = tri[i].t1c;
		tri_2[ (i * 3) + 0] = tri[i].t2a;
		tri_2[ (i * 3) + 1] = tri[i].t2b;
		tri_2[ (i * 3) + 2] = tri[i].t2c;
	}

		kernel = clCreateKernel(program, "left_match", NULL);
		if (kernel == NULL)
		{
			std::cerr << "Failed to create kernel" << std::endl;
			clCleanup(context, commandQueue, program, kernel, 0, 0);
			return ;
		}
	...
	size_t localWorkSize[1] = { 40 };
	size_t globalWorkSize[1] = { RoundUp(localWorkSize[0], tri.size()) };
	//size_t globalWorkSize[2] = { 64, 64};
	cout<<"width:::::"<<globalWorkSize[0];//<<",height::::"<<globalWorkSize[1];
	//size_t globalWorkSize[2] = {715*2,492};
	float t1=clock();
	...
	errNum = clEnqueueReadBuffer(commandQueue, memObjects[9], CL_TRUE,0, 715*492 * sizeof(float), D ,0, NULL, NULL);
	if (errNum != CL_SUCCESS)
	{
		std::cerr << "Error reading result buffer." << std::endl;
		std::cout<<"error number:"<<errNum<<endl;
		clCleanup(context, commandQueue, program, kernel, 0,0);
		return ;
	}
}
void clElas::clcomputeDisparityright(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
								uint8_t* I1_desc,uint8_t* I2_desc,float* D)
{
	const int32_t disp_num  = grid_dims[0]-1;
	if (param.subsampling) {
	...
	kernel = clCreateKernel(program, "right_match", NULL);
	if (kernel == NULL)
	{
		std::cerr << "Failed to create kernel" << std::endl;
		clCleanup(context, commandQueue, program, kernel, 0, 0);
	...
	printf("1111111111111111111111111111right match\n");
	...
	size_t globalWorkSize[1] = { RoundUp(localWorkSize[0], tri.size()) };
	//size_t globalWorkSize[2] = { 64, 64};
	cout<<"width:::::"<<tri.size();//<<",height::::"<<globalWorkSize[1];
	//size_t globalWorkSize[2] = {715*2,492};
	float t1=clock();
	// Queue the kernel up for execution
	errNum = clEnqueueNDRangeKernel(commandQueue, kernel, 1, NULL,
...
void clElas::discriptor(cl_mem *imageObjects,uint8_t *I1_desc)
{
	...
	if (errNum != CL_SUCCESS)
	{
		...
		clCleanup(context, commandQueue, program, kernel, imageObjects, sampler);
		return ;
	}
	
	// Create OpenCL kernel
	kernel = clCreateKernel(program, "descriptor", NULL);
	if (kernel == NULL)
	{
		std::cerr << "Failed to create kernel" << std::endl;
		clCleanup(context, commandQueue, program, kernel, imageObjects, sampler);
	...
	uint8_t* I_du = (uint8_t*)malloc(width*height*sizeof(uint8_t));
    uint8_t* I_dv = (uint8_t*)malloc(width*height*sizeof(uint8_t));
	
	// Set the kernel arguments

	memObjects[0] = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(uint8_t)*width*height,NULL,NULL);
	memObjects[1] = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(uint8_t)*width*height,NULL,NULL);
	
	errNum = clSetKernelArg(kernel, 0, sizeof(cl_mem), imageObjects);
	...
	errNum = clEnqueueReadBuffer(commandQueue, memObjects[0], CL_TRUE,0, width*height * sizeof(uint8_t), I_du,0, NULL, NULL);
    if (errNum != CL_SUCCESS)
    {
        std::cerr << "Error reading result buffer." << std::endl;
        clCleanup(context, commandQueue, program, kernel, memObjects,0);
        return ;
    }
	errNum = clEnqueueReadBuffer(commandQueue, memObjects[1], CL_TRUE,0, width*height * sizeof(uint8_t), I_dv,0, NULL, NULL);
    if (errNum != CL_SUCCESS)
    {
        std::cerr << "Error reading result buffer." << std::endl;
        clCleanup(context, commandQueue, program, kernel, memObjects,0);
	...
	for (int32_t v=3; v<height-3; v++) {
      ...
      for (int32_t u=3; u<width-3; u++) {
		  I_desc_curr = I1_desc+(v*width+u)*16;
	...
	t2=clock();
	cout << "cost time2222222 : " << t2-t1<<endl;
	std::cout << std::endl;
	std::cout << "Executed program succesfully." << std::endl;
	free(I_du);
	free(I_dv);
	
	clCleanup(0, 0, 0, 0, imageObjects, sampler);
}
void clElas::removeInconsistentSupportPoints (int16_t* D_can,int32_t D_can_width,int32_t D_can_height) {
...
void clElas::removeRedundantSupportPoints(int16_t* D_can,int32_t D_can_width,int32_t D_can_height,
...
void clElas::addCornerSupportPoints(vector<support_pt> &p_support) {
...
inline int16_t clElas::computeMatchingDisparity (const int32_t &u,const int32_t &v,uint8_t* I1_desc,uint8_t* I2_desc,const bool &right_image) {
...
vector<clElas::support_pt> clElas::computeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc) {
...
vector<clElas::triangle> clElas::computeDelaunayTriangulation (vector<support_pt> p_support,int32_t right_image) {
...
void clElas::computeDisparityPlanes (vector<support_pt> p_support,vector<triangle> &tri,int32_t right_image) {
...
void clElas::createGrid(vector<support_pt> p_support,int32_t* disparity_grid,int32_t* grid_dims,bool right_image) {
...
inline void clElas::updatePosteriorMinimum(__m128i* I2_block_addr,const int32_t &d,const int32_t &w,
...
inline void clElas::updatePosteriorMinimum(__m128i* I2_block_addr,const int32_t &d,
                                         const __m128i &xmm1,__m128i &xmm2,int32_t &val,int32_t &min_val,int32_t &min_d) {
  xmm2 = _mm_load_si128(I2_block_addr);
  xmm2 = _mm_sad_epu8(xmm1,xmm2);
...
inline void clElas::findMatch(int32_t &u,int32_t &v,float &plane_a,float &plane_b,float &plane_c,
  ...
  const int32_t disp_num    = grid_dims[0]-1;  //64
...
void clElas::computeDisparity(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
  ...
  for (uint32_t i=0; i<tri.size(); i++) {
    ...
    for (uint32_t j=0; j<3; j++) {
      for (uint32_t k=0; k<j; k++) {  /// this sort has error
        if (tri_u[k]>tri_u[j]) {
...
void clElas::leftRightConsistencyCheck(float* D1,float* D2) {
...
void clElas::removeSmallSegments (float* D) {
...
void clElas::gapInterpolation(float* D) {
...
void clElas::adaptiveMean (float* D) {
