...
public:
  ...
  clElas (parameters param) : param(param) {
	 context = 0;
	 commandQueue = 0;
	 program = 0;
	 device = 0;
...
private:
  ...
  //cl function
  void clElas::discriptor(cl_mem *imageObjects,uint8_t *I_desc);
//  vector<clElas::support_pt> clElas::clcomputeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc);
  void clElas::clcomputeDisparity(std::vector<support_pt> p_support,std::vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,uint8_t* I1_desc,uint8_t* I2_desc,float* D);
  void clElas::clcomputeDisparityright(std::vector<support_pt> p_support,std::vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,uint8_t* I1_desc,uint8_t* I2_desc,float* D);
  ...
  void process (uint8_t* I1,uint8_t* I2,float* D1,float* D2,const int32_t* dims);//, bool bCompSuppt, vector<support_pt> p_support);
